            ...
            {activeResult ? (
              <>
                ...
                <details className="answerTrace">
                  <summary>
                    <span><BrainCircuit size={17} /> How Ari Answered This</span>
                    <ChevronDown className="traceChevron" size={17} />
                  </summary>
                  <div className="traceTimeline" aria-label="AI technical details">
                    <div className="traceStep">
                      <CheckCircle2 size={18} />
                      <div><span>Intent detected</span><strong>{activeResult.model_trace?.label || "General"} inquiry</strong></div>
                    </div>
                    <div className="traceStep">
                      <CheckCircle2 size={18} />
                      <div><span>Selected agent</span><strong>{activeResult.agent_name || "Ari Campus Router"}</strong></div>
                    </div>
                    <div className="traceStep">
                      <CheckCircle2 size={18} />
                      <div><span>Data source</span><strong>{activeResult.data_path || "Verified campus tools"}</strong></div>
                    </div>
                    <div className="traceStep">
                      <CheckCircle2 size={18} />
                      <div>
                        <span>Response generated by</span>
                        <strong>{activeResult.llm_provider === "Google Gemini" ? `${activeResult.llm_model} + grounded campus evidence` : "Verified campus tools · Gemini fallback"}</strong>
                      </div>
                    </div>
                    <div className="traceStep">
                      <CheckCircle2 size={18} />
                      <div><span>Evidence retrieved</span><strong>{(activeResult.sources?.length || 0) + (activeResult.rows?.length || 0)} verified result(s)</strong></div>
                    </div>
                    <div className="traceStep">
                      <Clock3 size={18} />
                      <div><span>Latency</span><strong>{activeResult.response_time_ms ?? 0} ms</strong></div>
                    </div>
                  </div>

                  {!!activeResult.workflow?.length && (
                    <div className="traceWorkflow">
                      <strong>LangGraph workflow</strong>
                      <ol>
                        {activeResult.workflow.map((step) => <li key={step}>{step}</li>)}
                      </ol>
                    </div>
                  )}
                </details>
